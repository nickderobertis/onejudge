...
/// CLI carried `oneharness-core` 0.6.13, and the 0.13.0 CLI carries
/// `oneharness-core` 0.14.0 — so naming the core version here would tell an
...
const MIN_ONEHARNESS: &str = "0.13.0";

/// The `oneharness-core` version the [`MIN_ONEHARNESS`] CLI release embeds, read
/// off that release's own `oneharness-core` requirement (`oneharness` 0.13.0
/// depends on `^0.14.0`).
///
...
/// different crates, and 0.13.0 is the CLI carrying the required 0.14.0 core.
...
#[cfg(test)]
const MIN_ONEHARNESS_CORE: &str = "0.14.0";
